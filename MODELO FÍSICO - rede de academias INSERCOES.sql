...
-- CRIANDO ACESSOS ALEATORIOS
INSERT INTO acesso (data_acesso, id_aluno, id_unidade)
SELECT 
    CONCAT(
        DATE_ADD('2024-01-01', INTERVAL FLOOR(RAND() * 243) DAY), 
        ' ', 
        SEC_TO_TIME(FLOOR(8 * 3600 + RAND() * (15 * 3600)))
    ) AS data_acesso,
    FLOOR(1 + RAND() * 10) AS id_aluno,
    FLOOR(1 + RAND() * 6) AS id_unidade
FROM 
